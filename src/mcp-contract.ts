...
export const cicloMcpTools: readonly McpToolContract[] = [
  ...
  },
  {
    ...
    description: "Plan a Herdr-reachable remote runner on Kubernetes, AWS Lambda MicroVM, or Cloudflare with WireGuard tunnel setup and runtime preflight checks.",
    inputSchema: objectSchema("Remote runner launch request.", {
      ...
      bead_id: beadId,
      harness_id: harnessId,
      image: stringSchema("Runner image containing Ciclo harness dependencies. Optional when image_resolver is provided by config or request."),
      image_resolver: { type: "object", description: "Image resolver request. Built-ins are static, variant, and nixery; accepts strategy, image, registry, repository, tag, variant, base_packages, harness_packages, and extra_packages." },
