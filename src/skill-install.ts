...
- When Ciclo MCP is running inside Herdr, local Claude/Codex workers launch as visible Herdr agents by default. With \`isolation: "worktree"\`, Ciclo uses fresh \`herdr worktree create\` workspaces and starts the agent inside the returned Herdr workspace id; existing Herdr worktree paths fail instead of reopening stale workspace state. Ciclo MCP runs an internal heartbeat for sessions it owns; use \`ciclo_decide\` when a monitoring, context, question-answering, or operator-interface decision needs OpenAI judgment. Set \`CICLO_REUSE_HERDR_SESSION=false\` before starting MCP only when direct process launches are required.
