...
Use `worker_env` when the launched Claude/Codex worker shell needs non-secret values such as service URLs or mode flags. Project `mcp.vars` are inherited by worker shells automatically. Use `worker_secret_env` when the worker shell needs credentials for tools such as `gh`, `git`, or `curl`. These bindings use the same fields as `mcp_secret_env`, but Ciclo wraps the harness command itself so the worker process tree receives the variables. Do not use `mcp_secret_env` for shell credentials; those variables belong to the Ciclo MCP subprocess, not the agent shell.
