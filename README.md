...
When MCP is running, Ciclo elects one project/session MCP process as the automation leader. Only that leader starts the internal heartbeat; follower MCP subprocesses still answer tools and expose status, but they do not run heartbeat, preemptive work, cron, or cleanup loops. The heartbeat refreshes worker state, marks silent workers stale or stalled, checks registered remote sessions through Herdr remote attach when a remote heartbeat client is configured, evaluates configured cron jobs, compacts project memory, and asks the OpenAI brain for follow-up decisions instead of waiting for an external poll. `ciclo_status.mcp` reports whether the current MCP process is `leader`, `follower`, or `unmanaged`.
...
The config stores secret provider references, MCP defaults, heartbeat preemptive work limits, remote runner defaults, cron jobs, memory policy, and optional prompt guidance. It should contain provider ids, secret references, non-secret `vars`, and non-secret guidance text, not raw secret values. The heartbeat preemptive worker cap is `heartbeat.preemptiveWork.maxConcurrent`; when omitted, Ciclo defaults to launching at most 10 active worker sessions. Preemptive workers use `heartbeat.preemptiveWork.harnesses` as a pool; the built-in pool spreads work across Codex and Claude Code with `claude-fable-5`, while `heartbeat.preemptiveWork.harnessId` remains the fixed-harness override. When a preemptive launch does not explicitly set a model, Ciclo classifies the Beads problem by issue type, priority, labels, and title/description signals, then selects harness, model, and effort for the problem: small docs/chore work can use a smaller Codex model, standard bugs/tests use a standard Codex model, and hard orchestration/security/remote/review work prefers Claude Fable high effort when available. `bead.claimed`, `work.started`, and brain-decision context include the model-selection evidence. Ciclo first scans `heartbeat.preemptiveWork.issueTypes` and then falls back to `heartbeat.preemptiveWork.fallbackIssueTypes` so concrete task, bug, and decision Beads can continue after planning work is exhausted. `ciclo mcp install`, MCP runtime startup, spawned worker MCP setup, prompt builders, remote runner planning, cron scheduling, and memory compaction all read this file; explicit CLI or MCP tool arguments override config values.
...
- Worker process environment: use project `mcp.vars` or launch-time `worker_env` for non-secret values the launched Claude/Codex shell tools need, such as service URLs or mode flags. Use project `mcp.workerSecretBindings` or launch-time `worker_secret_env` for credentials, such as `gh`, `git`, or `curl` tokens. Ciclo wraps the harness command for secret values so the worker process tree receives resolved credentials, while generated MCP config files still contain no resolved secret values.
...
Supported runner kinds are `kubernetes`, `aws-lambda`, and `cloudflare`. Each kind is implemented by a remote runner provider plugin so new runnable environments can be added without changing the core planner. Kubernetes defaults to a StatefulSet for durable Herdr-attached worker sessions and falls back to a Job for `preflight_only` or explicit `kubernetes.mode: "job"` launches. Each plan includes provider artifacts or commands, generated remote MCP client config artifacts, image resolution evidence, repo bootstrap commands, project egress policy artifacts, a runtime preflight that checks Claude Code access and Ciclo-like build tools, WireGuard runner and host setup artifacts, the Herdr remote target reachable over that tunnel, and a `ciclo attach` plan for monitoring the overall Ciclo session.

Ciclo image resolution supports `static`, `variant`, and `nixery` strategies. Official image variants are built with Nix `dockerTools` as `base-latest`, `codex-latest`, `claude-latest`, and `full-latest`; CI smoke-tests each variant with `ciclo --version`. Use `preflight_only: true` to validate image, repo bootstrap, Claude access, and project dependencies before starting WireGuard or Herdr. Kubernetes launches clone `repo_url` into `repo_path` when needed and run `devenv shell -- true` when the project has `devenv.nix`, so project dependencies are loaded as the last mile in the remote environment. Configure `remote.egress.cidrs` and `remote.egress.domains` in `.ciclo/config.json`; Kubernetes emits NetworkPolicy for CIDR rules and annotates domain rules for clusters with CNI egress gateway or proxy enforcement. Configure `remote.wireGuard.hostRouting.serviceCidrs` for private service ranges that should route from the remote worker through the Ciclo host; Ciclo emits a host `wg-quick` config plus setup script that enables forwarding and NAT. If no key material or existing runner Secret is configured, Kubernetes plans include a bootstrap script that generates hub/runner keys locally and creates the Kubernetes `runner.conf` Secret.
